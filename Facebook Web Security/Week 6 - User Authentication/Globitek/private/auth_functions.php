  ...
  function require_login() {
    if(!is_logged_in()) {
     ...
     function record_failed_login($username) {
          //Getting the date of the last attempt
     ...
     function login_throttle($username) {
          $threshold = 5;     // Maximum amount of failed attempts
          $lockout = 5 * 60;  //Locks out user for 5 minutes after exceeding max number of logins

          //Finds the  failed login value for that username
          $failed_login_result = find_failed_login($username);
          $failed_login = db_fetch_assoc($failed_login_result);
          ...
          //Gets time from last attempt and determines whether user can log in
          ...
          //If 5 minutes haved passed, resets failed login counter
          if($remaining_lockout < 0) {
               reset_failed_login($username);
               return 0;
          }
          //Else returns the time left until it resets
          else {
               return $remaining_lockout;
          }
     }

     // Fetches the data from the database for resetting the failed login counter
     function reset_failed_login($username) {
          $failed_login_result = find_failed_login($username);
          $failed_login = db_fetch_assoc($failed_login_result);

          // Finds failed login record
          if($failed_login) {
