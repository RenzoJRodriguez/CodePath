<?php
  ...
  function validate_user($user, $errors=array()) {
       //Validating Update Password
       if(isset($user['new_password'])) {
            if(!is_blank($user['new_password'])) {
                 if(!password_verify($user['previous_password'], $user['hashed_password'])) {
  ...
  function delete_user($user) {
    ...
    if($result) {
      ...
      //
      // FAILED_LOGINS QUERIES
      //

     // Finds all users, ordered last_name then first_name
     ...
     // Finds failed login using username
     ...
     function insert_failed_login($failed_login) {
          ...
          if($result) {
               ...
               // If false, shows errors w/o form
               echo db_error($db);
               db_close($db);
               exit;
          }
     ...
     // Edits failed login record
     // Returns True or an array of errors
