...
* Bonus Objective 1\.
  * [x]  Identify the subtle Username Enumeration weakness. Include a short description of how the code could be modified to be more secure below:
            - The Username Enumeration can be found in the public/staff/login.php page. In this page there are 2 error messages when the page is 
              submitted and the query is performed for the finding user:
                1\. If the Username is found, but the password is incorrect: "Log in was unsuccessful."
                2\. If Username is not found: "Log in was not successful."
                
                This violates the fundamental security principle of "Security Through Obscurity". In violating this principle, the system is vulnerable for exploitation because a hacker can brute force the usernames and enumerate the ones that return "Log in was unsuccessful." It can be modified
                by making the second message the same as the first. This will remove the distinction between the two messages and prevent Username Enumeration.
...
* Bonus Objective 3\.
  ...
  * [x]  Briefly describe why login still works even after the cost is changed:
            - Since password_hash() returns the algorithm, cost and salt as part of the returned hash, 
              all the information that's needed to verify the hash is included in it.
...
<img src='http://i.imgur.com/KWB1apJ.gif' title='Video Walkthrough' width='' alt='Video Walkthrough' />
